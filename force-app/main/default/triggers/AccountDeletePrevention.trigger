trigger AccountDeletePrevention on Account(before delete) {
  ...
  AccountDeletePreventionHandler.handleBeforeDelete(Trigger.new, Trigger.oldMap);
...
// List<Opportunity> oppList= [Select AccountId, Name from Opportunity where AccountId in : Trigger.oldMap.keySet()];
// if(!oppList.isEmpty()){
