...
//        updateCase.Id  = userInfo.getUserId();
...
//     Scenario 3:
// If the subject of case contains "Cydeo" then update the status of case to Working only if the status is New,
//  if the status is not equals to New then do not update the status field of case object.

for(Case each: Trigger.new){
    if(each.Subject.contains('Cydeo') && each.Status=='New' && each.Status!='Working'){
       each.Status='Working ';
    }
